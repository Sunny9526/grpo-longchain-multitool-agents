...
> - SwanLab 训练曲线（历史项目名）：https://swanlab.cn/@godstear/agentic-grpo-longhorizon?utm_source=website_qr&utm_medium=qr_scan
>
...
![Ablation Multi-Dimensional Comparison](../../assets/figures/ablation_comparison.png)

**图注**：(a) Overall 成功率；(b) 泛化能力（uncovered + unseen）；(c) 工具错误率（越低越好）；(d) 单轮推理深度（p50 tokens）。联合方案在 overall 和泛化上均显著领先，error_rate 唯一成功降低，per-turn reasoning 深度与质量同步提升。图表由 [`results_summary.csv`](results_summary.csv) 生成。

#### 5.7.2 测评轨迹对比

![Training Progression](../../assets/figures/ablation_progression.png)

**图注**：Turn-Discount 在 200–300 step 几乎停滞，验证了其「消极保护」的局限；LATA 通过 √L 归一化获得持续提升；联合方案在 step250 达到峰值 0.240，step300 出现轻微回落。LATA step300 在旧绘图脚本中标记为估计值，新图以空心点区分。
