...
> **文档定位**：基于 [Vanilla GRPO 诊断](../vanilla-grpo/vanilla_grpo_diagnosis.md) 的三类问题，设计并实施四个消融实验。本文是「实验设计手册」——只包含**计划、设计原理、代码实现和预期指标**，不包含任何实际训练进度。
>
> **实际执行范围**：四个实验（Turn-Discount、PRM-Lite、LATA、PRM-Lite + LATA）。已删除未执行的 partial credit 方案。
>
> **实验结果**：见 [消融诊断报告](ablation_diagnosis_report.md) 和 [机器可读结果汇总](results_summary.csv)。
