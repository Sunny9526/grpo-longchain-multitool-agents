...
的目标是**暴露并记录 vanilla GRPO 的 4 类核心问题**(reward 稀疏 / long-horizon credit misassignment / 长度-entropy 漂移 / KL 失控)。为此 rollout 设计必须:
...
trainer:
  ...
  project_name: grpo-longchain-multitool-agents
...
完成后需要产出的文档:
- [`../experiments/vanilla-grpo/vanilla_grpo_diagnosis.md`](../experiments/vanilla-grpo/vanilla_grpo_diagnosis.md)——诊断 4 类核心问题(reward 稀疏 / credit misassignment / 长度-entropy 漂移 / KL 失控)
