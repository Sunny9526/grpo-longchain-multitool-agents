#!/bin/bash
# 批量评估 lata GRPO 的 3 个 checkpoint (hf_step_200/250/300)
#
# 前置条件：
...
# 如果 lata 有自己的 split.json，优先用它
if [ -f "experiments/lata/split.json" ]; then
    SPLIT_FILE="experiments/lata/split.json"
fi

echo "========================================"
echo "  Exp 2 LATA GRPO 批量评估"
echo "  Split file: ${SPLIT_FILE}"
echo "========================================"

for STEP in "${STEPS[@]}"; do
    MODEL_PATH="experiments/lata/hf_step_${STEP}"
    OUTPUT_DIR="experiments/lata/eval_step_${STEP}"
    CONFIG="configs/eval/lata/eval_lata_step${STEP}.yaml"
...
echo "  结果目录："
for STEP in "${STEPS[@]}"; do
    echo "    experiments/lata/eval_step_${STEP}/eval_report.json"
    echo "    experiments/lata/eval_step_${STEP}/split_eval_report.json"
done
echo "========================================"
